...
std::vector<int> floatingPoint::getBinaryExponent()
{
    ...
    if(decimalRepresentation < 2)
        exp = 127;
    std::vector<int>binaryExponent;
    convertIntegerToBinary(binaryExponent,exp);
...
void floatingPoint::mergeMantissa(std::vector<int> & mantissa, std::vector<int> & integerPart, std::vector<int> & fractionalPart)
{
    ...
    mantissa = normalizeMantissa(mantissa, integerPart.size());
...
std::vector<int> floatingPoint::normalizeMantissa(std::vector<int> mantissa, int integerSize)
{
    ...
    exponent = integerSize - dotPosition;
